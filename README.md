...
エコモコ丼は、著者が開発業務でディスコードの会議内容をよく忘れてしまうことや同じ音声をyoutubeに行き、何度も再生してしまうことから着想を得て開発。
音声を録音するだけでAIが自動で文字起こし・タグ付け・関連メモ分析を行い、メモ間の繋がりをインタラクティブなグラフで可視化。
ユーザーは意識せずにメモが整理され、時間抽出自動化による通知機能やソートにより、過去の知識が自然に浮かび上がる体験を得られる。
